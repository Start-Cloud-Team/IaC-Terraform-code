####################CodeBuild 생성####################
resource "aws_codebuild_project" "iac-codebuild"{
        ...
        phases:
            install:
                ...
                commands:
                    ...
                    - curl -Lo snyk https://static.snyk.io/cli/$IMAGE_TAG/snyk-linux
                    - chmod +x snyk
            ...
            pre_build:
                commands:
                    ...
                    - docker pull webgoat/webgoat:$IMAGE_TAG || true

            build:
                commands:
                    - echo "=== TAG IMAGE FOR ECR ==="
                    - docker tag webgoat/webgoat:$IMAGE_TAG $ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$IMAGE_REPO_NAME:$IMAGE_TAG
            ...
            post_build:
                commands:
                    ...
                    - >
                        printf '[{"name":"iac-webgoat-container","imageUri":"%s"}]'
                        "$ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$IMAGE_REPO_NAME:$IMAGE_TAG"
                        > imageDetail.json
                    - echo "=== CREATE taskdef.json ==="
                    - >
                        printf '{"family":"iac-webgoat-container","executionRoleArn":"arn:aws:iam::%s:role/iac-task-execution-role","taskRoleArn":"arn:aws:iam::%s:role/iac-task-role","networkMode":"awsvpc","requiresCompatibilities":["FARGATE"],"cpu":"512","memory":"2048","enableExecuteCommand":true,"containerDefinitions":[{"name":"iac-webgoat-container","image":"%s","essential":true,"environment":[{"name":"TZ","value":"%s"},{"name":"WEBGOAT_HOST","value":"%s"},{"name":"WEBWOLF_HOST","value":"%s"},{"name":"EXCLUDE_CATEGORIES","value":"%s"},{"name":"EXCLUDE_LESSONS","value":"%s"}],"portMappings":[{"containerPort":8080,"protocol":"tcp"}]}]}'
                        "$ACCOUNT_ID"
                        "$ACCOUNT_ID"
                        "$ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$IMAGE_REPO_NAME:$IMAGE_TAG"
                    ...
                    - >
                        printf 'version: 0.0\nResources:\n  - TargetService:\n      Type: AWS::ECS::Service\n      Properties:\n        TaskDefinition: "<TASK_DEFINITION>"\n        LoadBalancerInfo:\n          ContainerName: "iac-webgoat-container"\n          ContainerPort: 8080'
                        > appspec.yml
